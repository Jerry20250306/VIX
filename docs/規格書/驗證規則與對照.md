...
| :--- | :--- | :--- | :--- |
| **Near Term (Step 0)** | `step0_full_output_Near_全天.csv` | `NearPROD_YYYYMMDD.tsv` | 近月選擇權序列報價快照 |
| **Next Term (Step 0)** | `step0_full_output_Next_全天.csv` | `NextPROD_YYYYMMDD.tsv` | 次近月選擇權序列報價快照 |
| **VIX 計算 (Step 1)** | `my_sigma_YYYYMMDD.tsv` | `sigma_YYYYMMDD.tsv` | 最終 VIX 指數計算與變異數結果 |
...
Step 1 比對的是 `my_sigma_*.tsv` 與 PROD `sigma_*.tsv`，主要對照以下欄位，皆以 `Time` (如 084530) 作為主鍵：

| 驗證項目 | PROD 欄位 | 系統欄位 | 備註 |
| :--- | :--- | :--- | :--- |
| **近月變異數** | `nearSigma2` | `nearSigma2` | 允許誤差 < 1e-4 |
| **次近月變異數** | `nextSigma2` | `nextSigma2` | 允許誤差 < 1e-4 |
| **對外揭示 VIX** | `vix` | `vix` | 四捨五入至小數後兩位，9點前不可揭示（-1） |
| **背景運算 VIX** | `ori_vix` | `ori_vix` | 全精確度，供後續插補參考 |
