...
class DataPathManager:
    ...
    def __init__(self, raw_base_dir="資料來源", prod_base_dir="資料來源"):
        ...
        # 優先從環境變數讀取
        env_raw = os.environ.get("VIX_DATA_SOURCE")
        env_prod = os.environ.get("VIX_PROD_SOURCE")
        
        if env_raw:
            self.raw_base_dir = env_raw if os.path.isabs(env_raw) else os.path.join(self.project_root, env_raw)
        else:
            self.raw_base_dir = os.path.join(self.project_root, raw_base_dir)
            
        if env_prod:
            self.prod_base_dir = env_prod if os.path.isabs(env_prod) else os.path.join(self.project_root, env_prod)
        else:
